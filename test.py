...
from Utilities import load_and_resolve_json  # Import the utility to load and resolve JSON
...
def chat():
    ...
    try:
        ...
        result = run_flow_from_json(flow="demo_test.json",
            input_value="message",
    ...
    # Attempt to extract the text from a Message object using its __dict__
            if hasattr(output_data, "__dict__"):
        # Try to get the 'data' attribute from the Message object
               data_attr = output_data.__dict__.get("data", {})
               if isinstance(data_attr, dict) and "text" in data_attr:
                   answer = data_attr["text"]
               else:
